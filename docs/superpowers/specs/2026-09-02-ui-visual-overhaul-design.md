...
技术栈：HTML5 Canvas + 原生 JavaScript（ES Modules），无构建工具、无框架；UI 图标与主角皮肤引入 PNG，场内实体与场景保持程序化几何
...
3. 怪物、弹道、部署物、辅助武器与场景继续使用 Canvas 2D 程序化几何，不把“怪物数量会很多”的扩展成本转化为逐个出图成本。
...
- 2026-08-19 设计中的“纯 Canvas、零图片资源”规则仅在本期被**有意收窄**：UI 图标和主角皮肤改用 PNG；怪物、弹道、部署物和场景仍保持程序化几何，不与本期混合路线冲突。

### 0.2 修订记录（2026-09-02）

本轮围绕 UI/视觉改造逐条确认以下 8 项裁定：

1. **美术路线 = 混合（已确认）**：商店、升级、道具栏、图鉴使用 k3-256k 生成的 128×128 透明底 PNG；怪物、弹道、场景保持并升级程序化几何矢量。
...
- **混合美术路线（已确认）**：UI 图标与主角皮肤使用 k3-256k 的 128×128 透明底 PNG；怪物、弹道、场景和部署物使用程序化几何矢量。
...
- UI 图标与皮肤统一使用 k3-256k；怪物、弹道、部署物和场景不以图片代替程序化渲染。
...
- **圆形障碍 → 岩石**：碰撞仍使用原圆形半径；绘制改为不规则多边形，包含顶部高光、底部阴影和霓光细描边。每个障碍在生成时获得稳定 `id`，用 `hash(id)` 选择变体；圆形岩石使用 3 个变体、矩形对象使用 2 个变体（2–3 个变体范围与稳定选择规则已确认）。同一地图种子下同一障碍的变体不得因帧数变化。
- **矩形障碍 → 废弃车辆 / 混凝土块**：保留轴对齐碰撞盒；视觉使用分层色块、车窗/车轮或混凝土裂缝、霓光描边区分对象，不扩大阻挡范围。
- **商店 → 霓虹补给站**：保留 5 座位置与 90px 交互半径；外观改为棚屋造型、霓绿招牌描边和地面交互半径光环。光环以低成本 alpha/scale 脉动提示可交互，不阻挡移动或弹道；“店”字改为图标/短标签组合，避免依赖单一文字。
- 视觉变体只依赖障碍 `id` 与配置，不依赖 `Math.random()` 每帧重选；障碍物的逻辑字段、空间网格登记和碰撞测试保持原样。

### 5.3 目标：地形、边界与其余实体

- **地形**：地面改为暗色噪点草地。地图初始化时生成程序化离屏地形 tile，再按可见范围平铺；每帧只做贴图合成，不重新生成噪点。调色板变化时重建一次 tile，resize 不按每帧重建。
- **边界**：灰色矩形边框改为霓虹警戒线，保留世界边界和相机夹紧逻辑；颜色、发光强度和警示状态全部读取 palette。
- **地雷**：当前仓库没有独立 `mine` 实体模块；炸弹 AoE 仍由 `src/systems/combat.js:1-6` 处理。本设计为部署地雷/落点爆炸类实体登记统一 `mine` visual id：使用低矮几何底座、中心警示光与范围环，先解决视觉注册与 UI 图标契约，不改变当前炸弹伤害、范围或触发逻辑。
- **特斯拉球**：逻辑保持 `src/entities/teslaball.js:1-31` 的移动、周期电击和 2.5s 生命周期（已确认保留）；视觉从散落绘制改为注册表组合：中心电磁球、环绕电弧和周期闪电，颜色使用青霓 token。游戏内渲染调用 `drawVisual`/注册部件，不新增图片贴图。
- **直升机**：逻辑状态机与降落/登机规则保持 `src/entities/helicopter.js:1-30` 的 3s 降落、3s 登机和半径 60px（均已确认保留）；现有 `src/entities/helicopter.js:32-73` 的内联几何迁入注册表，升级为机身、旋翼、尾部、舷窗和登机进度光环的组合模型。旋翼继续按动画相位旋转，不改变救援计时。
...
- **地形**：暗色噪点草地必须预渲染为离屏 tile；运行帧只平铺/合成，禁止每帧重新生成噪点、路径或随机障碍变体。
- **图标**：`getVisualCanvas` 按 `id + size` 键控离屏缓存；相同图标和尺寸在同一场景内不得重复解码或重复生成离屏 Canvas。
- **`drawImage`**：图片只从解码缓存/离屏缓存绘制，数量受控；HUD 5 槽、商店列表、图鉴网格不得因重复 DOM 刷新而重复创建图片对象。场内怪物、弹道、部署物和场景优先走程序化几何，避免把大量 `drawImage` 引入战斗帧。
...
| 5 | 场景建模 | 岩石/废弃车辆/混凝土块/霓虹补给站/噪点草地/警戒线，以及地雷、特斯拉球、直升机注册表视觉（已确认顺序） |
...
- 截图核对场内火炮炮管追踪、围墙三档受损、三类辅助武器、岩石/车辆/补给站/地形/边界、地雷/特斯拉球/直升机的程序化建模；确认图鉴形象与游戏内同源。
...
- **程序化实体不改为逐怪图片**：怪物、弹道、场景、火炮、围墙、辅助武器、地雷、特斯拉球和直升机的场内视觉均由 Canvas 几何/部件注册表生成；只有 UI 图标与主角皮肤使用 PNG。
