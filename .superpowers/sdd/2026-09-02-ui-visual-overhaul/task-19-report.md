...
- `src/config/assets.js`
  - 导出 `SKIN_FRAME_ORDER` 数组，定义 4 列（down/left/right/up）× 2 行（frame 0/1）每帧 128×128 的切片坐标。
...
- 实机裁剪验证：Node 模拟测试确认 `drawVisual` 在 `right` 帧 0 裁剪参数到达 `drawImage(image, 256, 0, 128, 128, 84, 84, 32, 32)`；portrait 裁剪参数到达 `drawImage(image, 0, 0, 128, 128, 26, 26, 48, 48)`。

## 偏差/说明

1. **测试用例增量与基线**：
   - 任务 18 基线为 341 个通过用例。
   - 本任务新增 `test/skins.test.js`（+2 个 manifest/crop 用例，总数 5），`test/player.test.js`（+3 个动画/渲染回退用例，总数 13），总测试用例数严格增长为 346 个，全部绿灯。
